...
const TAG_STYLES = {
  'Personal':  { color: '#1D9E75', bg: 'rgba(29,158,117,0.08)', border: 'rgba(29,158,117,0.3)' },
  'Academic':  { color: '#3178C6', bg: 'rgba(49,120,198,0.08)', border: 'rgba(49,120,198,0.3)' },
  'Hackathon': { color: '#C17D2E', bg: 'rgba(193,125,46,0.08)', border: 'rgba(193,125,46,0.3)' },
  'Company':   { color: '#7B3FA0', bg: 'rgba(123,63,160,0.08)', border: 'rgba(123,63,160,0.3)' },
}
...
const PROJECTS = [
  ...
  {
    title: 'Job Application Tracker', tags: ['Personal'],
  ...
  {
    title: 'Pokédex App', tags: ['Academic'],
  ...
  {
    title: 'Responsive Landing Page', tags: ['Academic'],
  ...
  {
    title: 'Real Estate Scheduling Platform', tags: ['Company'],
  ...
  {
    title: 'Payment Gateway Solution', tags: ['Company'],
  ...
  {
    title: 'B2B E-Commerce Platform', tags: ['Company'],
  ...
  {
    title: 'Asset Tracking System', tags: ['Company'],
  ...
  {
    title: 'Mobile Tracking & Identity Verification', tags: ['Company'],
  ...
  {
    title: 'File Conversion Utility', tags: ['Company'],
...
function TagBadge({ tag }) {
  ...
  return (
    <span style={{ padding: '2px 9px', background: s.bg, border: `1px solid ${s.border}`, borderRadius: '100px', fontSize: '0.7rem', fontWeight: 700, color: s.color, whiteSpace: 'nowrap' }}>{tag}</span>
  )
}
    ...
    >
      ...
      <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '12px', flex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
          ...
          <div style={{ flex: 1 }}>
            <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text)', lineHeight: 1.3, marginBottom: '6px' }}>{p.title}</h3>
          ...
          <div style={{ display: 'flex', gap: '5px', flexShrink: 0 }}>
            {p.live && (
              <a href={p.live} target="_blank" rel="noopener noreferrer" style={{ width: '32px', height: '32px', background: p.accent, borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '0.75rem', fontWeight: 700, textDecoration: 'none', transition: 'opacity 0.2s' }}
                onMouseEnter={e => e.currentTarget.style.opacity = '0.8'}
                onMouseLeave={e => e.currentTarget.style.opacity = '1'}
                title="Live">↗</a>
            )}
            {p.github && (
              <a href={p.github} target="_blank" rel="noopener noreferrer" style={{ width: '32px', height: '32px', border: '1px solid var(--divider)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text2)', fontSize: '0.75rem', fontWeight: 700, textDecoration: 'none', transition: 'all 0.2s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = p.accent; e.currentTarget.style.color = p.accent }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--divider)'; e.currentTarget.style.color = 'var(--text2)' }}
                title="GitHub">⌥</a>
            )}
            {!p.live && !p.github && (
              <span style={{ fontSize: '0.68rem', color: 'var(--text3)', fontStyle: 'italic', padding: '6px 4px' }}>Private</span>
            )}
          </div>
        </div>
        ...
        {/* Description */}
        <p style={{ fontSize: '0.88rem', fontWeight: 500, color: 'var(--text2)', lineHeight: 1.65, flex: 1 }}>{p.desc}</p>

        {/* Tech pills */}
        <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginTop: 'auto' }}>
          {p.tech.map(t => (
            <span key={t} style={{ padding: '3px 9px', border: '1px solid var(--tag-border)', borderRadius: '100px', fontSize: '0.72rem', fontWeight: 600, color: 'var(--tag)', background: 'var(--bg)' }}>{t}</span>
          ))}
        </div>
      </div>
    </div>
...
export default function Projects() {
  ...
  return (
    <section id="projects" style={{ borderTop: '1px solid var(--divider)', background: 'var(--bg2)', width: '100%' }}>
      <div className="section-wrap">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '40px', flexWrap: 'wrap', gap: '20px' }}>
          <h2 className="section-title" style={{ margin: 0 }}>Selected <em>Work</em></h2>
            ...
            {filters.map(f => {
              ...
              return (
                <button key={f} onClick={() => setActive(f)} style={{
                  padding: '8px 18px',
                  background: isActive ? (s ? s.bg : 'var(--gold-bg)') : 'transparent',
                  border: `1.5px solid ${isActive ? (s ? s.border : 'var(--gold-border)') : 'var(--divider)'}`,
                  borderRadius: '100px', fontSize: '0.85rem', fontWeight: 700,
                  color: isActive ? (s ? s.color : 'var(--gold)') : 'var(--text3)',
                  cursor: 'pointer', transition: 'all 0.2s', fontFamily: 'inherit',
                }}
                  onMouseEnter={e => { if (!isActive) { e.currentTarget.style.borderColor = 'var(--gold-border)'; e.currentTarget.style.color = 'var(--gold)' } }}
                  onMouseLeave={e => { if (!isActive) { e.currentTarget.style.borderColor = 'var(--divider)'; e.currentTarget.style.color = 'var(--text3)' } }}
                >
                  {f}{f !== 'All' && <span style={{ opacity: 0.55, fontSize: '0.72rem', marginLeft: '4px' }}>({PROJECTS.filter(p => p.tags.includes(f)).length})</span>}
                </button>
              )
            })}
          </div>
